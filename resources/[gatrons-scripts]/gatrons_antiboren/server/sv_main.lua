...
function ToggleTrollProtection(target, toogleOverride, timeOverride)
    TriggerClientEvent("gatrons_antiboren:toggle", target, timeOverride)
end

RegisterNetEvent("gatrons_antiboren:updateTime", function(time)
    local timeLeft = time
...
RegisterNetEvent("gatrons_antiboren:onjoin", function()
    local source = source
    local identifier = GetPlayerIdentifier(source)
...
function onJoin(source, identifier, isNew)
    ...
    if time > 0 then
        TriggerClientEvent("gatrons_antiboren:toggle", source, true, time)
        return
    end
