...
function createTableIfNotExist()
    ...
    MySQL.query([[
    create table if not exists antiboren_time
    (
...
function insert(identifier, time)
    MySQL.query.await('INSERT IGNORE INTO antiboren_time (identifier, time_left) VALUES (?, ?)', { identifier, time })
end

function update(identifier, time)
    MySQL.query.await('update antiboren_time set time_left = ? where identifier = ?', { time, identifier })
end

function doesUserExist(identifier)
    local result = MySQL.query.await('select * from antiboren_time where identifier = ?', { identifier })
    if result[1] then
        return true
    else
        return false
