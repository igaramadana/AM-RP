...
RegisterNetEvent('gatrons_givecash:sendNotify')
AddEventHandler('gatrons_givecash:sendNotify', sendNotify)

createInputMenu = function(data)
    if not data.entity then
        print('data.entity: nil value')
        return
    end
    local totalMoney = lib.callback.await('gatrons_givecash:getPlayerMoney', false)
    if not (totalMoney > 0) then
        sendNotify('You don\'t have money!', 'error')
        return
    end
    ...
    local alert = lib.alertDialog({
        header = 'Give?',
        content = 'You give the amount of money $' ..
        lib.math.groupdigits(input[1]) .. (input[2] ~= '' and '  \nNotes : ' .. input[2] or ''),
        centered = true,
        cancel = true,
        size = 'md'
    })
    if alert == 'confirm' then
        ...
        TriggerServerEvent('gatrons_givecash:sendMoney', ap)
        handleInputDialog(false, data)
    end
end
